...
class LibTuringPosTests: XCTestCase {
    
    let gen = ProcessonGenSubroutine()
    ...
    let bundle = Bundle.init(for: LibTuringPosTests.self)

    override func setUp() {
        // Put setup code here. This method is called before the invocation of each test method in the class.
        guard let posPath = bundle.path(forResource: "FSM", ofType: "pos") else {
            fatalError("测试文件丢失")
        }
    ...
    func testGenCode() {
        guard let comparisonHeaderPath = bundle.path(forResource: "POSTestStateMachineHeader", ofType: "txt")
            else {
        ...
        guard let comparisonImplPath = bundle.path(forResource: "POSTestStateMachineImpl", ofType: "txt"),
